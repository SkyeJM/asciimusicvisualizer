...
let audioContext, analyser, source; // Web Audio API components
let isVisualizerRunning = false; // Flag to control visualizer updates
const baseFontSize = 7;
// Base font size for ASCII art
...
// Handle the audio file upload by reading it as an array buffer and decoding it:
...
// Create a function to start audio playback and initiate the visualizer:

document.getElementById('startVisualizer').addEventListener('click', startVisualizer);

function startVisualizer() {
  if (!currentImage || !audioBuffer) {
  ...
  analyser.fftSize = 256; // Number of samples for analysis
  ...
  updateASCIIArt(); // Begin updating the ASCII art
}
...
function generateASCII(img, threshold = null, blurValue = null) {
  const edgeMethod = document.querySelector('input[name="edgeMethod"]:checked').value;
  if (edgeMethod === 'dog') {
    generateContourASCII(img, threshold, blurValue);
    return;
  }
  ...
  const blur = blurValue !== null ? blurValue : parseFloat(document.getElementById('blur').value);
  ...
  let ascii = "";
  if (edgeMethod === 'sobel') {
    const edgeThreshold = threshold !== null ? threshold : parseInt(document.getElementById('edgeThreshold').value, 10);
    gray = applyEdgeDetection(gray, asciiWidth, asciiHeight, edgeThreshold);
...
function applyEdgeDetection(gray, width, height, threshold = null) {
  const edgeThreshold = threshold !== null ? threshold : parseInt(document.getElementById('edgeThreshold').value, 10);
  ...
  for (let y = 1; y < height - 1; y++) {
    for (let x = 1; x < width - 1; x++) {
      ...
      edges[idx] = normalized > edgeThreshold ? 0 : 255;
    }
  }
  return edges;
}
...
function generateContourASCII(img, threshold = null, blurValue = null) {
  const canvas = document.getElementById('canvas');
  const ctx = canvas.getContext('2d');
  const asciiWidth = parseInt(document.getElementById('asciiWidth').value, 10);
  const brightness = parseFloat(document.getElementById('brightness').value);
  ...
  const dogThreshold = threshold !== null ? threshold : parseInt(document.getElementById('dogEdgeThreshold').value, 10);

  let ascii = "";
  for (let y = 0; y < asciiHeight; y++) {
    let line = "";
    for (let x = 0; x < asciiWidth; x++) {
      if (suppressedMag[y][x] > dogThreshold) {
...
function updateASCIIArt() {
  ...
  for (let i = 0; i < timeData.length; i++) {
    const val = (timeData[i] - 128) / 128; // Convert to -1 to 1
    sum += val * val;
  }
  const rms = Math.sqrt(sum / timeData.length);
  const maxBlur = 10;
  const blurValue = rms * maxBlur; // Map amplitude to blur (0-10)

  // Get frequency data for bass energy
  const freqData = new Uint8Array(analyser.frequencyBinCount);
  analyser.getByteFrequencyData(freqData);
  let bassEnergy = 0;
  for (let i = 0; i < 10; i++) { // First 10 bins for bass
    bassEnergy += freqData[i];
  }
  bassEnergy /= 10;
  const maxThreshold = 255;
  const minThreshold = 0;
  const threshold = maxThreshold - (bassEnergy / 255) * (maxThreshold - minThreshold); // More bass -> lower threshold

  // Generate ASCII art with dynamic parameters
  generateASCII(currentImage, threshold, blurValue);

  // Schedule next update
  setTimeout(updateASCIIArt, 100); // Update every 100ms
}
